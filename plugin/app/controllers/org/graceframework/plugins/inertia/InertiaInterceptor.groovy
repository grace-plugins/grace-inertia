...
class InertiaInterceptor {
    ...
    boolean after() {
        ...
        if (isInertiaRequest()) {
            if (isGetRequest() && isVersionStale()) {
                header(InertiaRequest.X_INERTIA_LOCATION, webRequest.currentRequest.forwardURI)
                header(InertiaRequest.X_INERTIA_VERSION, assetVersion)
    ...
    private boolean isVersionStale() {
        String clientVersion = request.getHeader(InertiaRequest.X_INERTIA_VERSION)
